module ApplicationHelper
  ...
  def top_layout_css_class(params)
    return "#{params["controller"]} #{params["controller"]}_#{params['action']} rails_#{Rails.env()}";
  end

  def google_scholar_link(string)
    # articles
    return link_to('http://scholar.google.com/scholar?as_sdt=0,31&q=' + string.split.join('+'),
  ...
  def sherpa_romeo_journal(string, label = "SR")
    return link_to('http://www.sherpa.ac.uk/romeo/search.php?versions=all&prule=ALL&jrule=CONTAINS&search=' + string.split.join('+'),
                    :class => "admin_actions") do
      label
    end
  end
  ...
  def directory_link(email, label = "?-DIR")
      return link_to('http://search.princeton.edu/search?e=' + Rack::Utils.escape(email),
                     :class => "admin_actions") do
        label
      end
  end
  ...
  def institution_site_search(words, label = "?-PU")
    return link_to('http://www.princeton.edu/main/tools/search/?q=' + Rack::Utils.escape(words.split().join('+')),
                   :class => "admin_actions") do
      label
    end
  end
  ...
  def employee_list(name, label = "A")
    return link_to(search_get_employees_path(:search_term => name), :class => "admin_actions") do
      label
    end
  end
  ...
  def login_out_link(html_opts = {})
    ...
    return link_to(content_tag(:span, "", :class => "glyphicon glyphicon-user") + " " + label, link, html_opts)
  end
