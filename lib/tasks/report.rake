...
namespace :report do
  ...
  task :recent, [:max] => :environment do |t, args|
  ...
  namespace :waivers do
    ...
    task :count_legacy_with_props => :environment do |t, args|
      author_status_list = WaiverInfo.select(:author_status).uniq.collect { |w| w.author_status }
    ...
    task :print_faculty_waivers => :environment do |t, args|
      st = 'faculty'
