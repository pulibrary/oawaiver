...
RSpec.describe Employee, :type => :model do
  ...
  [:first_name, :last_name, :unique_id, :email, :department, :netid].each { |field|
  ...
  [:unique_id, :netid, :email].each { |field|
  ...
  [:netid, :email].each { |field|
    [:upcase, :downcase].each do |fct|
  ...
  it "search_all_by_name (unreliable solr test)" do
    ...
    ['Chemistry', "Future Past"].each do |dn|
      ['David', 'Maria', 'Jane'].each do |fn|
        ['Smith', 'Jones', 'Jones'].each do |ln|
          netid = "#{fn.downcase}#{i}"
          obj = Employee.create(first_name: fn, last_name: ln, unique_id: "0" + i.to_s,
                                department: dn,
                                netid: netid, email: "#{netid}@princeton.edu")
          obj.save!
          i += 1
        end
      end
    end
    obj = Employee.create(first_name: "Hannah", last_name: "David", unique_id: "1" + i.to_s,
                          department: "Physics",
                          netid: "xxx", email: "xxx@princeton.edu");
    obj.save!
    e = Employee.first
    e.preferred_name = "nowhere-else"
    e.save!
    Employee.all.each { |e| puts e.inspect }
    sleep(1)
    cases = {"Chemistry" => 0, "Jane" => 6, "David" => 7, "nowhere-else" => 1, 'Jones' => 12}
    cases.each do |word, cnt|
      puts [word, cnt].inspect
      matches = Employee.all_by_name(word)
  ...
  it "all_by_department (unreliable solr test)" do
    i = 0
    ['Comedy', 'Comedy', 'Science'].each do |dn|
    ...
    cases.each do |word, cnt|
      puts [word, cnt].inspect
      matches = Employee.all_by_department(word)
      matches.results.each { |e| puts "#{word}: #{e.inspect}" }
      expect(matches.results.count).to eq(cnt);
    end
  end
  ...
  it ("load excel file: spec/data/employee_2.csv") do
    logger = Lumberjack::Logger.new(device = STDOUT, options = {:level => :info})
    result = Employee.loadCsv("spec/data/employee_2.csv", logger)
  ...
  it ("load excel file: spec/data/employee_3_9_errors.csv") do
    logger = Lumberjack::Logger.new(device = STDOUT, options = {:level => :info})
    result = Employee.loadCsv("spec/data/employee_3_9_errors.csv", logger)
