...
RSpec.describe WaiverInfo, :type => :model do
  ...
  it "has a valid factory" do
    expect(FactoryGirl.build(:waiver_info)).to be_valid
  end

  [:author_first_name, :author_last_name, :author_email, :author_department, :author_status,
   :title, :journal, :requester, :requester_email].each { |field|
    it ("can't create without " + field.to_s) do
      waiver = FactoryGirl.build(:waiver_info, field => "");
  ...
  it ("can't create for faculty without unique_id") do
    waiver = FactoryGirl.build(:waiver_info, :author_status => AuthorStatus.StatusFaculty, :author_unique_id => nil);
    expect(waiver.valid?).to eq(false)
    expect(waiver.errors.messages[:author_unique_id]).not_to eq(nil);
  end

  [:journal_issn, :author_unique_id, :notes].each { |field|
    it ("can create without " + field.to_s) do
      waiver = FactoryGirl.build(:waiver_info, field => nil);
      expect(waiver.valid?).to eq(true)
    end
  }

  it ("can create for non faculty without unique_id") do
    waiver = FactoryGirl.build(:waiver_info, :author_status => AuthorStatus.StatusOther, :author_unique_id => nil);
    expect(waiver.valid?).to eq(true)
  end
  ...
  bad_emails.each do |email|
    it ("can't create with email  ''" + email + "'") do
      waiver = FactoryGirl.build(:waiver_info, :author_email => email);
      expect(waiver.valid?).to eq(false)
      expect(waiver.errors.messages[:author_email]).not_to eq(nil);
    end

    it ("can't create with requester_email ''" + email + "'") do
      waiver = FactoryGirl.build(:waiver_info, :requester_email => email);
      expect(waiver.valid?).to eq(false)
  ...
  it ("can create with author_unique_id '" + good_id + "'") do
    waiver = FactoryGirl.build(:waiver_info, :author_unique_id => good_id);
    expect(waiver.valid?).to eq(true)
  end

  bad_unique_ids = ["1234-6789", "1234567899", "12345678", "aberdeftg"];
  bad_unique_ids.each do |bad_id|
    it ("can't create with unique_id '" + bad_id + "'") do
      obj = FactoryGirl.build(:waiver_info, :author_unique_id => bad_id);
